...
RT-INCV-017 Kiểm tra tính toán công nợ khách hàng khi tạo hóa đơn
    [Documentation]    Kiểm tra tính toán công nợ khách hàng khi tạo hóa đơn:
    ...    - Khách hàng có công nợ ban đầu: 500,000đ
    ...    - Tạo hóa đơn với giá trị: 300,000đ
    ...    - Công nợ sau khi tạo hóa đơn: 800,000đ
    [Tags]    invoice    validation    debt    calculation
    Given Chuẩn bị dữ liệu hóa đơn với khách hàng có công nợ ${invoice_code_validation_customer_debt}
    When Gửi yêu cầu tạo hóa đơn
    Then Response Status Code Should Be 200
    And Response Should Have CustomerDebt With value ${invoice_code_validation_customer_debt_after}

RT-INCV-018 Kiểm tra tính toán công nợ khách hàng khi tạo hóa đơn thanh toán một phần
    [Documentation]    Kiểm tra tính toán công nợ khách hàng khi tạo hóa đơn thanh toán một phần:
    ...    - Khách hàng có công nợ ban đầu: 500,000đ
    ...    - Tạo hóa đơn với giá trị: 300,000đ
    ...    - Thanh toán: 100,000đ
    ...    - Công nợ sau khi tạo hóa đơn: 700,000đ
    [Tags]    invoice    validation    debt    calculation    partial
    Given Chuẩn bị dữ liệu hóa đơn thanh toán một phần với khách hàng có công nợ ${invoice_code_validation_customer_debt}
    When Gửi yêu cầu tạo hóa đơn
    Then Response Status Code Should Be 200
    And Response Should Have CustomerDebt With value ${invoice_code_validation_customer_debt_partial}

RT-INCV-019 Kiểm tra tính toán công nợ khách hàng khi tạo hóa đơn thanh toán đầy đủ
    [Documentation]    Kiểm tra tính toán công nợ khách hàng khi tạo hóa đơn thanh toán đầy đủ:
    ...    - Khách hàng có công nợ ban đầu: 500,000đ
    ...    - Tạo hóa đơn với giá trị: 300,000đ
    ...    - Thanh toán: 300,000đ
    ...    - Công nợ sau khi tạo hóa đơn: 500,000đ
    [Tags]    invoice    validation    debt    calculation    full
    Given Chuẩn bị dữ liệu hóa đơn thanh toán đầy đủ với khách hàng có công nợ ${invoice_code_validation_customer_debt}
    When Gửi yêu cầu tạo hóa đơn
    Then Response Status Code Should Be 200
    And Response Should Have CustomerDebt With value ${invoice_code_validation_customer_debt_before}

RT-INCV-020 Kiểm tra tính toán công nợ khách hàng khi tạo hóa đơn trả trước
    [Documentation]    Kiểm tra tính toán công nợ khách hàng khi tạo hóa đơn trả trước:
    ...    - Khách hàng có công nợ ban đầu: 500,000đ
    ...    - Tạo hóa đơn với giá trị: 300,000đ
    ...    - Thanh toán: 400,000đ
    ...    - Công nợ sau khi tạo hóa đơn: 400,000đ
    [Tags]    invoice    validation    debt    calculation    prepaid
    Given Chuẩn bị dữ liệu hóa đơn trả trước với khách hàng có công nợ ${invoice_code_validation_customer_debt}
    When Gửi yêu cầu tạo hóa đơn
    Then Response Status Code Should Be 200
    And Response Should Have CustomerDebt With value ${invoice_code_validation_customer_debt_prepaid}

RT-INCV-021 Kiểm tra tính toán công nợ khách hàng khi tạo hóa đơn thanh toán toàn bộ công nợ
    [Documentation]    Kiểm tra tính toán công nợ khách hàng khi tạo hóa đơn thanh toán toàn bộ công nợ:
    ...    - Khách hàng có công nợ ban đầu: 500,000đ
    ...    - Tạo hóa đơn với giá trị: 300,000đ
    ...    - Thanh toán: 800,000đ
    ...    - Công nợ sau khi tạo hóa đơn: 0đ
    [Tags]    invoice    validation    debt    calculation    full_debt
    Given Chuẩn bị dữ liệu hóa đơn thanh toán toàn bộ công nợ với khách hàng có công nợ ${invoice_code_validation_customer_debt}
    When Gửi yêu cầu tạo hóa đơn
    Then Response Status Code Should Be 200
    And Response Should Have CustomerDebt With value 0