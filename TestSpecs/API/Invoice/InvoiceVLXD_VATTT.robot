...
RT-VLXD-001 Tạo hóa đơn có hàng hóa vật liệu xây dựng
    ...
    Given Chuẩn Bị Dữ Liệu Hóa Đơn Với Sản Phẩm ${PRODUCT_ID_VLXD} Và Kích Thước 3x4x4x4
    When Gửi Yêu Cầu Tạo Hóa Đơn
    Then Mã trạng thái phải là 200
    And Nội dung phản hồi trả về phải tồn tại Id
    And Xác Thực Kích Thước 3x4x4x4 Sản Phẩm ${PRODUCT_ID_VLXD} Trong DB
...
RT-VLXD-002 Tạo hóa đơn có nhiều sản phẩm VLXD với kích thước khác nhau
    ...
    Given Chuẩn Bị Dữ Liệu Hóa Đơn Với Sản Phẩm ${PRODUCT_ID_VLXD_2} Kích Thước 100x50x20x5 Và ${PRODUCT_ID_VLXD_3} Kích Thước 200x100x30
    When Gửi Yêu Cầu Tạo Hóa Đơn
    Then Mã trạng thái phải là 200
    And Nội dung phản hồi trả về phải tồn tại Id
    And Xác Thực Kích Thước 100x50x20x5 Sản Phẩm ${PRODUCT_ID_VLXD_2} Trong DB
    And Xác thực Sản Phẩm Gạch ${PRODUCT_ID_VLXD_3} Có Kích Thước 200x100x30 Trong DB
...
RT-VLXD-003 Tạo hóa đơn VLXD là hàng gạch
    ...
    Given Chuẩn Bị Dữ Liệu Hóa Đơn Với Sản Phẩm Gạch ${PRODUCT_ID_VLXD_2} Và Kích Thước 30x40x5
    When Gửi Yêu Cầu Tạo Hóa Đơn
    Then Mã trạng thái phải là 200
    And Nội dung phản hồi trả về phải tồn tại Id
    And Xác thực Sản Phẩm Gạch ${PRODUCT_ID_VLXD_2} Có Kích Thước 30x40x5 Trong DB
...
RT-VLXD-004 Tạo hóa đơn VLXD Có nhiều dòng hàng
    ...
    ...    - Thanh toán được ghi nhận chính xác
    [Tags]    invoice    vlxd    payment
    Given Chuẩn Bị Dữ Liệu Hóa Đơn Với Sản Phẩm ${PRODUCT_ID_VLXD_2} Có 10 Dòng Với Kích Thước 30x40x5
    When Gửi Yêu Cầu Tạo Hóa Đơn
    Then Mã trạng thái phải là 200
    And Nội dung phản hồi trả về phải tồn tại Id
    And Xác Thực Sản Phẩm ${PRODUCT_ID_VLXD_2} Có 10 Dòng Với Kích Thước 30x40x5 Trong DB
...
RT-VLXD-005 Tạo hóa đơn VLXD gợi ý nhiều dòng
    ...
    ...    - Thanh toán được ghi nhận chính xác
    [Tags]    invoice    vlxd    payment
    Given Chuẩn Bị Dữ Liệu Hóa Đơn Với Sản Phẩm ${PRODUCT_ID_VLXD_2} Có 5 Gợi ý và Kích Thước 30x40x5
    When Gửi Yêu Cầu Tạo Hóa Đơn
    Then Mã trạng thái phải là 200
    And Nội dung phản hồi trả về phải tồn tại Id
    And Xác Thực Sản Phẩm ${PRODUCT_ID_VLXD_2} Có 5 Dòng Với Kích Thước 30x40x5 Trong DB
...
RT-DP-006 Tạo hóa đơn có tích điểm theo hóa đơn
    ...
    ...    - Dữ liệu đầu vào: 
    ...    - Sản phẩm: ID=${PRODUCT_ID_VLXD_2}, Số lượng=1
    ...    - Khách hàng 
    ...
    Given Chuẩn Bị Dữ Liệu Hóa Đơn Với Sản Phẩm ${PRODUCT_ID_VLXD_2} Với Khách Hàng ${CUSTOMER_ID_WITH_LOYALTY}
    When Gửi Yêu Cầu Tạo Hóa Đơn
    Then Mã trạng thái phải là 200
    And Nội dung phản hồi trả về phải tồn tại Id
...
RT-DP-007 Tạo hóa đơn có tích điểm theo hóa đơn giảm giá
    ...
    ...    - Điểm tích lũy được ghi nhận chính xác trong DB
    [Tags]    invoice    loyalty    points    vlxd
    Given Chuẩn Bị Dữ Liệu Hóa Đơn Tích Điểm Với Sản Phẩm ${PRODUCT_ID_VLXD_2} Có Giảm Giá 10000 và Khách Hàng ${CUSTOMER_ID_WITH_LOYALTY_2}
    When Gửi Yêu Cầu Tạo Hóa Đơn
    Then Mã trạng thái phải là 200
    And Nội dung phản hồi trả về phải tồn tại Id
