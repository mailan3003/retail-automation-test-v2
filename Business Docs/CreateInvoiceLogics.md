...
### 1.1 Kiểm tra và xử lý mã hóa đơn
#### 1.1.1. Kiểm tra hóa đơn trùng lặp khi đồng bộ offline
- Luồng kiểm tra trùng:
  + Kiểm tra cache Redis với key "cache:InvoiceCode:{RetailerId}:{Code}"
  + Query database nếu không có trong cache
- Điều kiện xác định trùng:
  + Cùng RetailerId
  + Cùng mã hóa đơn hoặc UUID trong khoảng 7 ngày trước/sau
  + Trạng thái hóa đơn khác Void và Failed
- Xử lý khi phát hiện trùng:
  + Đánh dấu hóa đơn với IsDuplicated = true
  + Lưu mã gốc vào trường CompareCode
  + Ghi log vào AuditTrail:
    * ModuleName = "Invoice" 
    * ActionName = "DuplicateCode"
    * Description = Chi tiết hóa đơn trùng
    * RetailerId = ID đơn vị

#### 1.1.2. Xử lý prefix khác nhau cho hóa đơn
- Prefix theo nguồn hóa đơn:
  + HDO: Hóa đơn offline
  + ShopeePrefix: Hóa đơn Shopee
  + LazadaPrefix: Hóa đơn Lazada
  + FacebookPrefix: Hóa đơn Facebook
  + InstagramPrefix: Hóa đơn Instagram
  + TikiPrefix: Hóa đơn Tiki
  + SendoPrefix: Hóa đơn Sendo
  + WarrantyPrefix: Hóa đơn bảo hành

#### 1.1.3. Sinh mã hóa đơn tự động theo định dạng từng loại
- Format mã theo loại:
  + Hóa đơn thường: {Prefix} + số tự tăng
  + Hóa đơn update: {UpdatePrefix} + mã gốc + "." + số thứ tự
  + Hóa đơn clone: {ClonePrefix} + mã gốc + "." + số thứ tự
  + Hóa đơn thay thế: Giữ nguyên mã gốc trong CompareCode

- Trường hợp đặc biệt:
  + FBPos: Thêm "FB_" vào đầu mã
  + Instagram: Thêm "IG_" vào đầu mã
  + Tiktok: Thêm "TT_" vào đầu mã

#### 1.1.4. Xử lý trường hợp hóa đơn bị trùng mã
- Kiểm tra và sinh mã mới:
  + Kiểm tra mã đã tồn tại trong cùng RetailerId
  + Với hóa đơn offline (HDO):
    * Cho phép trùng mã
    * Thêm số thứ tự sau dấu chấm
    * VD: HDO123 -> HDO123.1
  + Với hóa đơn thường:
    * Tăng số thứ tự cuối cùng
    * Kiểm tra lại đến khi không còn trùng

- Validate quyền:
  + Kiểm tra quyền Invoice.UpdateComplete khi thay đổi mã
  + Chỉ admin hoặc user có quyền mới được cập nhật 
  + Kiểm tra trạng thái hóa đơn gốc không phải Void/Failed

- Lưu vết và cache:
  + Lưu mã gốc vào CompareCode để trace
  + Cache thông tin trong Redis:
    * Key: cache:InvoiceCode:{RetailerId}:{Code}
    * TTL: 7 ngày
  + Ghi log AuditTrail khi có thay đổi mã

- Ràng buộc chung:
  + Độ dài mã tối đa 50 ký tự
  + Không cho phép trùng mã giữa các RetailerId
  + Validate format mã theo quy tắc từng loại
  + Đánh dấu IsDuplicated = true với hóa đơn trùng
