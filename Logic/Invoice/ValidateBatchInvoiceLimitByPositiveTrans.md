# Phân tích Business Logic của phương thức ValidateBatchInvoiceLimitByPositiveTrans

## Tổng quan

- **Mục đích**: Kiểm tra tồn kho theo giao dịch dương cho các lô sản phẩm trong hóa đơn
- **Đầu vào**: 
  ...
  + oldBatchIds: Danh sách ID các lô cũ (nếu đang cập nhật hóa đơn)
...
### 3. Xử lý các lô cũ (nếu đang cập nhật hóa đơn)

```csharp
if (oldBatchIds != null && oldBatchIds.Any())
{
    var oldInvoiceDetails = await Db.InvoiceDetails
        .Where(id => id.InvoiceId == invoice.UpdateInvoiceId &&
                     oldBatchIds.Contains(id.ProductBatchExpireId ?? 0))
        .ToDictionaryAsync(id => id.ProductBatchExpireId ?? 0, id => id.Quantity);

    foreach (var oldBatchId in oldBatchIds)
    {
        if (dictClosestOnhand.ContainsKey(oldBatchId))
        {
            dictClosestOnhand[oldBatchId] += oldInvoiceDetails[oldBatchId];
        }
    }
}
```

- Nếu đang cập nhật hóa đơn và có lô cũ:
  + Lấy thông tin số lượng của các lô cũ từ chi tiết hóa đơn cũ
  + Cộng thêm số lượng này vào số tồn kho hiện tại của các lô tương ứng
  + Mục đích: Trả lại số lượng cho các lô cũ trước khi kiểm tra tồn kho
...
2. **Cập nhật hóa đơn**:
   ...
   - Cộng thêm số lượng từ hóa đơn cũ vào số tồn kho
   - Đảm bảo không bị trừ số lượng hai lần
